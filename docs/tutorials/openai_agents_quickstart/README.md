...
- `openai-agents` available. Either install the optional extra:
  ```bash
  uv add 'agora-agent[openai-agents]'
  ```
...
| --- | --- |
| `ModuleNotFoundError: No module named 'agents'` | `openai-agents` not installed in this env. `uv add 'agora-agent[openai-agents]'` or `uv pip install openai-agents`. |
