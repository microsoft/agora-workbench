...
from connector import DispatcherConfig, WorkerConfig
from connector.dispatcher import DispatcherServer, _get_session_id_from_context
...
class TestDispatcherCatalogSync:
    ...
    async def test_fetches_catalog_from_first_worker(self, two_worker_config):
        ...
        with patch("connector.dispatcher.httpx.AsyncClient") as mock_client_cls:
...
class TestDispatcherProxyCall:
    ...
    async def test_proxy_call_success(self, two_worker_config):
        ...
        with patch("connector.dispatcher.httpx.AsyncClient") as mock_client_cls:
            ...
            with patch("connector.dispatcher.get_current_request_token", return_value="token-123"):
    ...
    async def test_proxy_call_marks_unhealthy_on_failure(self, two_worker_config):
        ...
        with patch("connector.dispatcher.httpx.AsyncClient") as mock_client_cls:
            ...
            with patch("connector.dispatcher.get_current_request_token", return_value=None):
                result = await server._proxy_dispatcher_call(
                    upstream=upstream,
                    tool_name="execute_code",
