...
:class:`~agora.agent.AgoraAgent`, ``ModularAgent`` is **not** a
...
``agora/README.md`` for narrative documentation and examples.
"""
...
class ModularAgent:
    ...
    public interface as :class:`~agora.agent.AgoraAgent` but does
    ...
    * The MAF workflow runs interactively, pausing on ``HelpResponse`` to
    ...
    ``agora/README.md`` for worked examples.
    ...
    Args:
        ...
        enable_toolmaker (bool):
            ...
            the :class:`~toolmaker.ToolMakerAgent` sub-agent.
            Default: ``False``.
