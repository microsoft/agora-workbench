...
def get_assm_cands(mol, atoms, inter_label, cluster, inter_size):
    ...
    if inter_size == 1:
        ...
        cands = [pos] + [
            (x, y) for x, y in cands if (rank[min(x, y)], rank[max(x, y)]) != (rank[min(pos)], rank[max(pos)])
        ]
    else:
        shift = cluster[inter_size - 1 :] + cluster[: inter_size - 1]
...
def check_pattern(mol, pattern):
    matches = mol.GetSubstructMatches(pattern)
    return len(matches) == 2 and len(matches[0]) == 3 and len(matches[1]) == 3


def check_acid(smiles):
    pattern = Chem.MolFromSmarts("[CX3](=O)[OX2H1]")
    if smiles != "":
        mol = Chem.MolFromSmiles(smiles)
        if mol is not None and check_pattern(mol, pattern):
            return True
    return False


def check_epoxide(smiles):
    pattern = Chem.MolFromSmarts("[OD2r3]1[#6D2r3][#6r3]1")
    if smiles != "":
        mol = Chem.MolFromSmiles(smiles)
        if mol is not None and check_pattern(mol, pattern):
            return True
