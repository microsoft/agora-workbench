...
from vitrimer_vae_tools.vae.chemutils import *
from vitrimer_vae_tools.vae.nnutils import get_device
...
class IncBase(object):
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=200, max_nb=12):
        ...
        self.fnode = torch.zeros(max_nodes * batch_size, node_fdim).long().to(get_device())
...
class IncGraph(IncBase):
    ...
    def add_mol(self, batch_idx, smiles, inter_label, nth_child):
        ...
        for bond in emol.GetBonds():
            ...
            if existing_bond is None:
                ...
                self.add_edge(
                    a1, a2, self.get_mess_feature(bond.GetBeginAtom(), bond_type, nth_child if a2 in attached else 0)
                )  # only child to father node (in intersection) have non-zero nth_child
                self.add_edge(
                    a2, a1, self.get_mess_feature(bond.GetEndAtom(), bond_type, nth_child if a1 in attached else 0)
                )
            else:
    ...
    def get_mess_feature(self, atom, bond_type, nth_child):
        ...
        f3[nth_child] = 1
        return torch.cat([f1, f2, f3], dim=-1).to(get_device())
