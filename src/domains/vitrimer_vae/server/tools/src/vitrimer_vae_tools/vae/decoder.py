...
from vitrimer_vae_tools.vae.encoder import IncHierMPNEncoder
from vitrimer_vae_tools.vae.mol_graph import MolGraph
from vitrimer_vae_tools.vae.inc_graph import IncTree, IncGraph
...
class HierMPNDecoder(nn.Module):
    def __init__(
        self, vocab, avocab, rnn_type, embed_size, hidden_size, latent_size, depthT, depthG, dropout, attention=False
    ):
        ...
        self.itensor = torch.LongTensor([]).to(get_device())
        ...
        self.E_order = torch.eye(MolGraph.MAX_POS).to(get_device())

        self.topoNN = nn.Sequential(
            nn.Linear(hidden_size + latent_size, hidden_size), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hidden_size, 1)
        )
        ...
        self.cls_loss1 = nn.CrossEntropyLoss(size_average=True)
        self.icls_loss1 = nn.CrossEntropyLoss(size_average=True)
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        hinter = HTuple(
            mess=self.rnn_cell.get_init_state(inter_tensors[1]), emask=self.itensor.new_zeros(inter_tensors[1].size(0))
        )
        hgraph = HTuple(
        ...
        for t in range(maxt):
            ...
            htree, hinter, hgraph = self.hmpn(
                cur_tree_tensors, cur_inter_tensors, cur_graph_tensors, htree, hinter, hgraph, subtree, subgraph
            )
    ...
    def decode(self, src_mol_vecs, greedy=True, max_decode_step=100, beam=5):
        ...
        graph_batch = IncGraph(
            self.avocab, batch_size, node_fdim=self.hmpn.atom_size, edge_fdim=self.hmpn.atom_size + self.hmpn.bond_size
        )
        ...
        for t in range(max_decode_step):
            ...
            htree, hinter, hgraph = self.hmpn(
                tree_tensors, tree_tensors, graph_tensors, htree, hinter, hgraph, subtree, subgraph
            )
