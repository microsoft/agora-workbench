...
from vitrimer_vae_tools.vae.encoder import HierMPNEncoder
from vitrimer_vae_tools.vae.decoder import HierMPNDecoder
from vitrimer_vae_tools.vae.nnutils import *
...
class HierVAE(nn.Module):
    def __init__(self, args):
        ...
        self.encoder_aci = HierMPNEncoder(
            args.vocab_aci,
            args.atom_vocab,
            args.rnn_type,
            args.embed_size,
        ...
        self.decoder_aci = HierMPNDecoder(
            args.vocab_aci,
            args.atom_vocab,
            args.rnn_type,
            args.embed_size,
        ...
        self.encoder_epo = HierMPNEncoder(
            args.vocab_epo,
            args.atom_vocab,
            args.rnn_type,
            args.embed_size,
        ...
        self.decoder_epo = HierMPNDecoder(
            args.vocab_epo,
            args.atom_vocab,
            args.rnn_type,
            args.embed_size,
        ...
        self.propNN = nn.Sequential(
            nn.Linear(args.latent_size, args.prop_hidden_size), nn.Linear(args.prop_hidden_size, 1)
        )
    ...
    def encode(self, batch, train=False):
        ...
        z_mean = torch.cat(
            (z_mean_aci_spec, (z_mean_aci_share + z_mean_epo_share) / 2, z_mean_epo_spec), dim=1
        )  # 8 cat 48 cat 8, 64
        z_log_var = torch.cat(
            (z_log_var_aci_spec, (z_log_var_aci_share + z_log_var_epo_share) / 2, z_log_var_epo_spec), dim=1
        )  # 8 cat 48 cat 8, 64
        if train:
    ...
    def repar_trick(self, z_mean, z_log_var):
        epsilon = torch.randn_like(z_mean).to(get_device())
    ...
    def sample(self, batch_size):
        z = torch.randn(batch_size, self.latent_size).to(get_device())
    ...
    def forward(self, batch, beta, beta_prop, prop=False):
        z_mean, z_log_var, graphs_aci, tensors_aci, orders_aci, graphs_epo, tensors_epo, orders_epo = self.encode(
            batch, train=True
        )
        ...
        z_aci = z[:, : self.acid_size]  # 0:56, 56
        z_epo = z[:, (self.latent_size - self.epoxide_size) :]  # 8:64, 56
        loss_aci, wacc_aci, iacc_aci, tacc_aci, sacc_aci = self.decoder_aci(
            (z_aci, z_aci, z_aci), graphs_aci, tensors_aci, orders_aci
        )
        loss_epo, wacc_epo, iacc_epo, tacc_epo, sacc_epo = self.decoder_epo(
            (z_epo, z_epo, z_epo), graphs_epo, tensors_epo, orders_epo
        )
        if prop:
            batch_prop = batch[2]
            batch_prop = torch.tensor(batch_prop).float().to(get_device())
            batch_prop_pred = self.propNN(z).squeeze()
            loss_prop = self.prop_loss(batch_prop_pred, batch_prop)
        else:
            loss_prop = torch.tensor(0).float().to(get_device())
