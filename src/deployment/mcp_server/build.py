...
this script inside ``deployment/mcp_server/``).  Override when the ``domains/``
...
"""Directory containing this script (src/deployment/mcp_server/)."""
...
class DomainConfig(BaseModel):
    ...
    module: str = Field(description="Python module entry-point, e.g. 'domains.example.server.example_server'")
    ...
    volumes: list[str] = Field(
        default_factory=list,
        description="Extra volume mounts for docker-compose",
    )
    ...
    memory_limit: str | None = Field(default=None, description="Compose memory limit, e.g. '32g'")
...
def _render_dockerfile(root: Path, output_dir: Path) -> Path:
    ...
    sections: list[str] = [
        ...
        "# Run: uv run python src/deployment/mcp_server/build.py generate",
        "#",
...
def _render_compose(root: Path, output_dir: Path) -> Path:
    ...
    trusted_service_names = ",".join(cfg.service_name for _, cfg in all_domains if cfg.trusted_hosts)
...
def cmd_new(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"       uv run python src/deployment/mcp_server/build.py generate\n"
        f"  4. Build the image:\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  uv run python src/deployment/mcp_server/build.py new chemistry\n"
        ),
    )
