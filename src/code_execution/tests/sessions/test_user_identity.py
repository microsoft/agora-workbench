...
class TestUserTokenKernelPropagation:
    ...
    async def test_execute_code_returns_notice_when_session_missing(self):
        ...
        stdout, stderr, success, _artifacts = await manager.execute_code_for_session(session_id, "print('hello')", timeout=1.0)
    ...
    async def test_execute_code_touches_session_during_long_running_execution(self):
        ...
        with patch.object(session, "touch", wraps=session.touch) as touch_spy:
            stdout, stderr, success, _artifacts = await manager.execute_code_for_session(session_id, "print('hello')", timeout=5.0)
    ...
    async def test_execute_code_returns_busy_when_background_job_running(self):
        ...
        stdout, stderr, success, _artifacts = await manager.execute_code_for_session(session_id, "print('blocked')", timeout=1.0)
        assert success is False
        assert stdout == ""
